...
class GestionOutliersUnivariados:
    ...
    def eliminar_outliers(self, metodo="iqr", limite_outliers=1.5):
        """
        Elimina outliers del DataFrame utilizando el método especificado.

        Parámetros:
        -----------
        metodo (str): Método para detectar outliers: "iqr" o "z_score".
        limite_outliers (float): Límite para definir los outliers en el método IQR. Por defecto es 1.5.

        Returns:
        --------
        DataFrame: DataFrame sin los outliers detectados.
        """
        df_cleaned = self.dataframe.copy()

        if metodo == "iqr":
            for columna in df_cleaned.select_dtypes(include=np.number).columns:
                ...
                df_cleaned = df_cleaned[(df_cleaned[columna] >= limite_inferior) & (df_cleaned[columna] <= limite_superior)]

        elif metodo == "z_score":
            from scipy.stats import zscore
            # Calcular el z-score y eliminar los outliers con |z| > 3
            for columna in df_cleaned.select_dtypes(include=np.number).columns:
                z_scores = zscore(df_cleaned[columna])
                df_cleaned = df_cleaned[np.abs(z_scores) <= 3]

        else:
            raise ValueError("Método no válido. Los métodos disponibles son 'iqr' y 'z_score'.")
...
class GestionOutliersMultivariados:
    ...
    def eliminar_outliers_multivariados(self, metodo="ifo", contaminacion=0.05):
        """
        Elimina outliers multivariados del DataFrame utilizando el método especificado.

        Parámetros:
        -----------
        metodo (str): Método para detectar outliers: "isolation_forest" o "lof".
        contaminacion (float): Proporción de datos que se consideran outliers. Por defecto es 0.05.
        ...
        """
        df_cleaned = self.dataframe.copy()
        col_numericas = df_cleaned.select_dtypes(include=np.number).columns

        if metodo == "ifo":
            isolation_forest = IsolationForest(contamination=contaminacion, random_state=42)
            outliers = isolation_forest.fit_predict(df_cleaned[col_numericas])
            df_cleaned = df_cleaned[outliers != -1]

        elif metodo == "lof":
            lof = LocalOutlierFactor(n_neighbors=20, contamination=contaminacion)
            outliers = lof.fit_predict(df_cleaned[col_numericas])
            df_cleaned = df_cleaned[outliers != -1]
